...
# --- Database Connection Dialog (No changes needed here) ---
class DbDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.db_type_input.setDisabled(True)
    ...
    def get_db_info(self):
        return {"db_type": self.db_type_input.text(), "db_name": self.db_name_input.text(), "table_name": self.table_name_input.text()}
    def on_import(self):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.df = pd.DataFrame({
            ...
            'Info': ['Use the buttons on the right to manage your data.']
        })
        self.model = PandasModel(self.df)
        self.table_view.setModel(self.model)
        # Add table to the main layout
        self.main_layout.addWidget(self.table_view, 4) # Add with a stretch factor of 4

        # --- Right Side: Button Panel ---
        # Create a container widget and a vertical layout for the buttons
        ...
        self.btn_fetch_api = QPushButton("Fetch from API")
        
        # Style for buttons for better visibility
        self.btn_db_ops.setStyleSheet("background-color: #cce5ff; padding: 5px;")
        self.btn_fetch_api.setStyleSheet("background-color: #d4edda; padding: 5px;")


        # Add buttons to the vertical button layout
        self.button_layout.addWidget(self.btn_import_csv)
        self.button_layout.addWidget(self.btn_import_excel)
        self.button_layout.addSpacing(20) # Add some space
        ...
        self.button_layout.addWidget(self.btn_db_ops)
        self.button_layout.addWidget(self.btn_fetch_api)
        self.button_layout.addStretch() # Pushes all buttons to the top

        # Set a fixed width for the button panel
        self.button_panel.setFixedWidth(200)

        # Add the button panel to the main horizontal layout
    ...
    # All other methods (show_message, import_csv, etc.) remain unchanged
    ...
    def export_csv(self):
        if self.df.empty:
            self.show_message("Info", "There is no data to export.")
            return
    ...
    def import_from_db(self, db_info):
        db_type, db_name, table_name = db_info['db_type'], db_info['db_name'], db_info['table_name']
